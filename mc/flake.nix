  ...
  outputs =
    ...
    in
    {
      packages = eachSystem (
        ...
        let
          ...
          mc-compat-valence-survival-furnace-persistence = pkgs.writeShellApplication {
            ...
            text = ''
              ...
              exec mc-compat-runner "$mode"                 --server-backend valence                 --scenario survival-furnace-persistence                 --receipt "$receipt"                 "$@"
            '';
            meta = {
        ...
        in
        {
          inherit valence stevenarella mc-compat-runner mc-compat-valence-ctf-600s-soak mc-compat-valence-ctf-blue-600s-soak mc-compat-valence-ctf-inventory-interaction mc-compat-valence-ctf-combat-damage mc-compat-valence-ctf-combat-knockback mc-compat-valence-ctf-armor-equipment-mitigation mc-compat-valence-ctf-equipment-update-observation mc-compat-valence-ctf-projectile-hit mc-compat-valence-ctf-projectile-damage-attribution mc-compat-valence-ctf-flag-carrier-death-return mc-compat-valence-ctf-latency-jitter-inventory mc-compat-valence-ctf-reconnect-flag-state mc-compat-valence-ctf-invalid-pickup-ownership mc-compat-valence-ctf-invalid-return-drop mc-compat-valence-ctf-score-limit-win-condition mc-compat-valence-ctf-simultaneous-pickup-capture-race mc-compat-valence-ctf-spawn-team-balance-reset mc-compat-valence-survival-break-place-pickup mc-compat-valence-survival-crafting-table mc-compat-valence-survival-furnace-persistence mc-compat-mcp-controlled-smoke;
...
' fake > fake-valence/README.md
        ...
        mc-compat-valence-ctf-spawn-team-balance-reset-dry-run =
          pkgs.runCommand "mc-compat-valence-ctf-spawn-team-balance-reset-dry-run" { nativeBuildInputs = [ pkgs.git ]; } ''
            mkdir -p fake-stevenarella fake-valence receipts
            printf '%s\n' '[package]' 'name = "stevenarella"' 'version = "0.0.0"' 'edition = "2021"' > fake-stevenarella/Cargo.toml
            git -C fake-valence init
            git -C fake-valence config user.email mc-compat@example.invalid
        ...
        mc-compat-valence-survival-furnace-persistence-dry-run =
          pkgs.runCommand "mc-compat-valence-survival-furnace-persistence-dry-run" { nativeBuildInputs = [ pkgs.git ]; } ''
            mkdir -p fake-stevenarella fake-valence receipts
            printf '%s\n' '[package]' 'name = "stevenarella"' 'version = "0.0.0"' 'edition = "2021"' > fake-stevenarella/Cargo.toml
            git -C fake-valence init
            git -C fake-valence config user.email mc-compat@example.invalid
