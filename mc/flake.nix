    ...
    in
    {
      ...
      checks = eachSystem (pkgs: {
        ...
        mc-compat-valence-inventory-stack-split-merge-dry-run =
          pkgs.runCommand "mc-compat-valence-inventory-stack-split-merge-dry-run" { nativeBuildInputs = [ pkgs.git ]; } ''
            mkdir -p fake-stevenarella fake-valence receipts
            printf '%s\n' '[package]' 'name = "stevenarella"' 'version = "0.0.0"' 'edition = "2021"' > fake-stevenarella/Cargo.toml
            git -C fake-valence init
            git -C fake-valence config user.email mc-compat@example.invalid
          ...
          '';
        mc-compat-inventory-stack-split-merge-evidence-check = pkgs.runCommand "mc-compat-inventory-stack-split-merge-evidence-check" { nativeBuildInputs = [ pkgs.rustc pkgs.gcc ]; } ''
          cp -R ${./.} repo
          chmod -R u+w repo
          cd repo
