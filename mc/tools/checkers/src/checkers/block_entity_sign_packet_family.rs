use crate::key_value::{
    ...
    require_exact, require_ok, KeyValueChecker, KeyValueRecord, ValidationResult, TRUE_VALUE,
};
...
pub struct BlockEntitySignPacketFamilyChecker;
...
impl KeyValueChecker for BlockEntitySignPacketFamilyChecker {
...
mod tests {
    ...
    const EXPECTED_OVERCLAIM_DIAGNOSTIC: &str = "broad overclaim claim.all_block_entities=true";

    #[test]
    fn valid_fixture_passes() {
        let evidence = Evidence::parse(&valid_fixture()).expect("fixture parses");
    ...
    fn negative_fixture_rejects_overclaim() {
        let text = format!("{}\nclaim.all_block_entities=true", valid_fixture());
        let evidence = Evidence::parse(&text).expect("fixture parses");
        let diagnostics = validate_evidence(&evidence).expect_err("overclaim fails closed");
        assert!(
            diagnostics
                .iter()
                .any(|diagnostic| diagnostic == EXPECTED_OVERCLAIM_DIAGNOSTIC),
            "{diagnostics:?}"
        );
    }
    ...
    fn self_test_runs_positive_and_negative_fixtures() {
        run_self_test().expect("self-test passes");
    }
}
