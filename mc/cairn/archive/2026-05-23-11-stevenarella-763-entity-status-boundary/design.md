# Design: EntityStatusS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x1c` as Valence `EntityStatusS2CPacket`. Its inherited fallback is inherited 758 Explosion_VarInt. This package covers only `0x1c` / `EntityStatusS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x1c` clientbound play mapping/parser needed for `EntityStatusS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x1c` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `EntityStatusS2CPacket` may require a packet-shape update rather than a simple ID remap.
