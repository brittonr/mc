# Design: PlayerListS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x3a` as Valence `PlayerListS2CPacket`. Its inherited fallback is inherited 758 EntityDestroy. This package covers only `0x3a` / `PlayerListS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x3a` clientbound play mapping/parser needed for `PlayerListS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x3a` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `PlayerListS2CPacket` may require a packet-shape update rather than a simple ID remap.
