# Design: UpdateSelectedSlotS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x4d` as Valence `UpdateSelectedSlotS2CPacket`. Its inherited fallback is inherited 758 EntityMetadata. This package covers only `0x4d` / `UpdateSelectedSlotS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x4d` clientbound play mapping/parser needed for `UpdateSelectedSlotS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x4d` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `UpdateSelectedSlotS2CPacket` may require a packet-shape update rather than a simple ID remap.
