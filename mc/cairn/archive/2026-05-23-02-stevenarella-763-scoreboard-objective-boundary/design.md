# Design: ScoreboardObjectiveUpdateS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x58` as Valence `ScoreboardObjectiveUpdateS2CPacket`. Its inherited fallback is inherited 758 TitleSubtitle. This package covers only `0x58` / `ScoreboardObjectiveUpdateS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x58` clientbound play mapping/parser needed for `ScoreboardObjectiveUpdateS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x58` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `ScoreboardObjectiveUpdateS2CPacket` may require a packet-shape update rather than a simple ID remap.
