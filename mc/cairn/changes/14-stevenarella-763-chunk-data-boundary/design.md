# Design: ChunkDataS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x24` as Valence `ChunkDataS2CPacket`. Its inherited fallback is inherited 758 Particle_f64. This package covers only `0x24` / `ChunkDataS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x24` clientbound play mapping/parser needed for `ChunkDataS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x24` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `ChunkDataS2CPacket` may require a packet-shape update rather than a simple ID remap.
