# Design: ScoreboardDisplayS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x51` as Valence `ScoreboardDisplayS2CPacket`. Its inherited fallback is inherited 758 SetExperience. This package covers only `0x51` / `ScoreboardDisplayS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x51` clientbound play mapping/parser needed for `ScoreboardDisplayS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x51` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `ScoreboardDisplayS2CPacket` may require a packet-shape update rather than a simple ID remap.
