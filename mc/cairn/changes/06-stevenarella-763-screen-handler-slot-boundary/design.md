# Design: ScreenHandlerSlotUpdateS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x14` as Valence `ScreenHandlerSlotUpdateS2CPacket`. Its inherited fallback is inherited 758 WindowItems_StateCarry. This package covers only `0x14` / `ScreenHandlerSlotUpdateS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x14` clientbound play mapping/parser needed for `ScreenHandlerSlotUpdateS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x14` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `ScreenHandlerSlotUpdateS2CPacket` may require a packet-shape update rather than a simple ID remap.
