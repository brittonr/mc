# Design: SynchronizeTagsS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x6e` as Valence `SynchronizeTagsS2CPacket`. Its inherited fallback is none / unmapped by 758 fallback. This package covers only `0x6e` / `SynchronizeTagsS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x6e` clientbound play mapping/parser needed for `SynchronizeTagsS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x6e` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `SynchronizeTagsS2CPacket` may require a packet-shape update rather than a simple ID remap.
