# Design: HealthUpdateS2CPacket protocol 763 boundary
...
The latest trace `/tmp/valence-763-after-game-message-update-trace.json` observed `0x57` as Valence `HealthUpdateS2CPacket`. Its inherited fallback is inherited 758 UpdateSimulationDistance. This package covers only `0x57` / `HealthUpdateS2CPacket`.

## Decisions

### 1. Keep the update boundary-first

**Choice:** Implement only the `0x57` clientbound play mapping/parser needed for `HealthUpdateS2CPacket` before moving to later trace packets.
...
**Choice:** The evidence receipt must claim only the `0x57` mapping update and must keep full-current-Valence and full-Stevenarella-763 support flags false.
...
- `HealthUpdateS2CPacket` may require a packet-shape update rather than a simple ID remap.
