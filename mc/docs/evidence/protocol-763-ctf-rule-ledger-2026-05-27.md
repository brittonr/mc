...
| --- | --- | --- |
| invalid_pickup_accepted | Non-claim | Add live invalid pickup/ownership receipt. |
| invalid_return_accepted | Non-claim | Add live invalid return/drop receipt. |
...
Only the three bounded rule clusters above are promoted. Full CTF correctness remains blocked until every rule-family row has live receipts, run logs, BLAKE3 manifests, matrix/current-bundle entries, and positive/negative checker coverage.

## Decision

- Question: Can current rule evidence be promoted without implying full CTF correctness?
- Inspected evidence: RED/BLUE historical oracle, flag-carrier death/return receipt, reconnect flag-state receipt, acceptance matrix, current bundle, and checker fixtures.
- Decision: Yes. Promote three bounded rule clusters and keep full CTF correctness as a non-claim.
- Owner: agent.
- Next action: add live invalid-action and race-condition receipts before broadening rule correctness.
