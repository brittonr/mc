...
| invalid_opponent_base_return_drop_fixture_without_state_mutation | CTF invalid-action breadth fixture | opponent-base-return-drop-without-carrier, ctf_invalid_opponent_base_return_drop_attempted, ctf_invalid_opponent_base_return_drop_contained | server_invalid_opponent_base_return_drop_rejected | unexpected_flag_pickup, unexpected_flag_return, unexpected_flag_capture, unexpected_red_score, unexpected_blue_score | `docs/evidence/protocol-763-ctf-invalid-opponent-base-return-drop-2026-06-19.receipt.json` | `b14786d1408fc3136284cfd83f68e8b8648d52576641784ebf003ba479a9f797` |
...
| invalid_return_accepted broad invalid return/drop breadth | Non-claim | One live own-base return/drop rejection row and one deterministic fixture-only opponent-base return/drop row are promoted; add more live carrier/drop/return permutations before claiming broader invalid return/drop coverage. |
...
- `tools/check_ctf_invalid_action_breadth.rs` validates the promoted deterministic fixture-only opponent-base return/drop row, including matrix-defined actor/flag teams, client containment, server rejection, no flag state mutation, no score/capture, and BLAKE3-backed logs;
...
Only the nine bounded rule clusters above are promoted. The invalid-action breadth fixture row is deterministic fixture evidence, not live gameplay evidence. Full CTF correctness remains blocked until every rule-family row has live receipts, run logs, BLAKE3 manifests, matrix/current-bundle entries, and positive/negative checker coverage.

## Decision

- Question: Can current rule evidence be promoted without implying full CTF correctness?
- Inspected evidence: RED/BLUE live-refresh receipts/logs, flag-carrier death/return receipt, reconnect flag-state receipt, invalid pickup ownership receipt, invalid return/drop receipt, invalid-action breadth fixture receipt, score-limit win-condition receipt, simultaneous pickup/capture race receipt, spawn/team balance/resource reset receipt, acceptance matrix, current bundle, and checker fixtures.
