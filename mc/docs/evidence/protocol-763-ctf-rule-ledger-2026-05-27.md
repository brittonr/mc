...
| invalid_opponent_base_return_drop_live_without_state_mutation | CTF invalid-action breadth live | opponent-base-return-drop-without-carrier, ctf_invalid_opponent_base_return_drop_attempted, ctf_invalid_opponent_base_return_drop_contained | server_invalid_opponent_base_return_drop_rejected | unexpected_flag_pickup, unexpected_server_flag_pickup, unexpected_flag_return, unexpected_flag_capture, unexpected_red_score, unexpected_blue_score | `docs/evidence/protocol-763-ctf-invalid-opponent-base-return-drop-live-2026-06-22.receipt.json` | `7739b00935d90f4d1cdb495d18d0430a3bf9c13b1a8b14dfdd46f90ef77bbfbb` |
...
| invalid_return_accepted broad invalid return/drop breadth | Non-claim | One live own-base return/drop rejection row and one bounded owned-local live opponent-base return/drop row are promoted; add more live carrier/drop/return permutations before claiming broader invalid return/drop coverage. |
...
- `tools/check_ctf_invalid_action_breadth.rs` validates the promoted bounded owned-local live opponent-base return/drop row, including matrix-defined actor/flag teams, client containment, server rejection, typed-event pass/fail contribution, no flag state mutation, no score/capture, and BLAKE3-backed logs;
...
- missing row id, mismatched actor team, missing server rejection, forbidden transitions, malformed manifest rows, and overclaims in `tools/check_ctf_invalid_action_breadth.rs --self-test`;
...
Only the nine bounded rule clusters above are promoted. The invalid-action breadth live row is bounded owned-local evidence for `opponent-base-return-drop-without-carrier`, not broad invalid-action breadth. Full CTF correctness remains blocked until every rule-family row has live receipts, run logs, BLAKE3 manifests, matrix/current-bundle entries, and positive/negative checker coverage.

## Decision

- Question: Can current rule evidence be promoted without implying full CTF correctness?
- Inspected evidence: RED/BLUE live-refresh receipts/logs, flag-carrier death/return receipt, reconnect flag-state receipt, invalid pickup ownership receipt, invalid return/drop receipt, invalid-action breadth live receipt, score-limit win-condition receipt, simultaneous pickup/capture race receipt, spawn/team balance/resource reset receipt, acceptance matrix, current bundle, and checker fixtures.
- Decision: Yes. Promote nine bounded rule clusters and keep full CTF correctness as a non-claim.
- Owner: agent.
...
No full CTF correctness, all invalid actions, all flag pickup/return permutations, live CTF semantics breadth, all score limits/settings, overtime/tiebreakers, scoreboard UI parity, all simultaneous races, latency/jitter race tolerance, all team balancing algorithms, all maps, all resource loadouts, all reset triggers, production gameplay readiness, public-server safety, or broad Minecraft compatibility claim is made.
