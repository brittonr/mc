...
Only the four bounded rule clusters above are promoted. Full CTF correctness remains blocked until every rule-family row has live receipts, run logs, BLAKE3 manifests, matrix/current-bundle entries, and positive/negative checker coverage.

## Decision

- Question: Can current rule evidence be promoted without implying full CTF correctness?
- Inspected evidence: RED/BLUE historical oracle, flag-carrier death/return receipt, reconnect flag-state receipt, invalid pickup ownership receipt, acceptance matrix, current bundle, and checker fixtures.
- Decision: Yes. Promote four bounded rule clusters and keep full CTF correctness as a non-claim.
- Owner: agent.
