...
`full_survival_compatibility` remains a non-claim because the world persistence row is still missing paired reference evidence beyond the currently covered break/place/pickup, crafting table, chest persistence, furnace persistence, hunger/food, mob drops, redstone toggle, and biome/dimension join state rows. In plain terms: full_survival_compatibility remains a non-claim.
