...
- projectile damage attribution, projectile travel, collision simulation, exact vanilla damage parity, or all bow/crossbow/trident use semantics,
...
1. **Projectile damage attribution rail** — requires pinned/reviewable Valence instrumentation and a causal client/server receipt that orders client projectile use/hit/damage evidence rather than accepting milestone presence alone.
2. **Projectile travel/collision simulation rail** — extends the indexed projectile use/loadout rail only if client-visible travel/collision evidence can be correlated with Valence server evidence.
3. **Armor loadout/enchantment matrix rail** — extends the indexed armor mitigation rail across more loadouts or modifiers without claiming full vanilla parity.
4. **Equipment slot/item matrix rail** — extends the indexed equipment update observation rail across additional slots and item types.
