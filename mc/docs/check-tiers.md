...
| `tier.evidence` | Evidence and receipt validation | `docs/evidence/` receipts, `.b3` manifests, task closeout evidence, promotion plans, and current evidence bundles | Evidence manifest check, task-evidence gate when tasks change, affected receipt/schema checker | Medium | Does not create new gameplay evidence; validates durability and claim boundaries only |
